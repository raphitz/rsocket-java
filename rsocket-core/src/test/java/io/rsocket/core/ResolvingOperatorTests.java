...
public class ResolvingOperatorTests {
  ...
  static Stream<Function<ResolvingTest<String>, Publisher<String>>> innerCases() {
    return Stream.of(
        self ->
            new ResolvingOperator.MonoDeferredResolution<String, String>(self) {
        ...
        self ->
            new ResolvingOperator.FluxDeferredResolution<String, String>(self) {
              final MonoProcessor<String> processor = MonoProcessor.create();
  ...
  public void shouldBeSerialIfRacyMonoInner() {
    for (int i = 0; i < 10000; i++) {
      ...
      ResolvingOperator.MonoDeferredResolution resolution =
          new ResolvingOperator.MonoDeferredResolution(ResolvingTest.create()) {

            @Override
            public void accept(Object o, Object o2) {}
          };

      resolution.subscribe(AssertSubscriber.create(0));
  ...
  @Test
  public void shouldBeSerialIfRacyFluxInner() {
    for (int i = 0; i < 10000; i++) {
      long[] requested = new long[] {0};
      Subscription mockSubscription = Mockito.mock(Subscription.class);
      Mockito.doAnswer(
          ...
          .when(mockSubscription)
          .request(Mockito.anyLong());
      ResolvingOperator.FluxDeferredResolution resolution =
          new ResolvingOperator.FluxDeferredResolution(ResolvingTest.create()) {
  ...
  static class ResolvingTest<T> extends ResolvingOperator<T> {
    ...
    public ResolvingTest<T> assertPendingSubscribers(int cnt) {
      Assertions.assertThat(this.subscribers).hasSize(cnt);
